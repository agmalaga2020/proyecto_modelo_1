...
# # Corrección de Integración con Formato Adecuado de CPRO
# 
# Este script corrige el problema de integración asegurando que los códigos de provincia (CPRO)
# se formateen correctamente como strings con ceros a la izquierda antes del mapeo.
...
# CORRECCIÓN CLAVE: Convertir CPRO a string con formato de dos dígitos (con cero a la izquierda)
print("\nConvirtiendo CPRO a string con formato adecuado...")
municipal_df['CPRO_str'] = municipal_df['CPRO'].astype(str).str.zfill(2)

# Verificar la conversión
print("Ejemplos de CPRO convertidos:")
print(municipal_df[['CPRO', 'CPRO_str']].drop_duplicates().sort_values('CPRO').head(10))
...
# Añadir el nombre de la provincia al dataset municipal usando CPRO_str
municipal_df['provincia'] = municipal_df['CPRO_str'].map(provincia_codes)

# Verificar la asignación de nombres de provincia
print("\nVerificando la asignación de nombres de provincia:")
print(municipal_df[['CPRO', 'CPRO_str', 'provincia']].drop_duplicates().sort_values('CPRO').head(10))
...
print(f"Ejemplos de provincias: {sorted([p for p in unique_provinces if not pd.isna(p)])[:10]}")
...
# Convertir columnas a tipos numéricos para el análisis
