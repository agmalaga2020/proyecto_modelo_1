...
# # Modelo Predictivo Simplificado de Precios de Vivienda con Variables Socioeconómicas
# 
# Este script realiza un análisis simplificado para identificar las variables más importantes
# para predecir el precio de la vivienda, utilizando un conjunto reducido de modelos.
...
import logging
logging.basicConfig(
    filename=f'{results_dir}/modelo_log.txt',
    level=logging.INFO,
...
try:
    ...
    log_progress("Cargando dataset integrado...")
    df = pd.read_csv('housing_municipal_integrated_fixed.csv')

    # Verificar las dimensiones
    log_progress(f"Dimensiones del dataset: {df.shape}")
    log_progress(f"Porcentaje de filas con datos socioeconómicos: {(df['poblacion_total'].notna().sum() / len(df)) * 100:.2f}%")
    ...
    log_progress(f"Filas disponibles para modelo con variables socioeconómicas: {len(df_with_socioeconomic)}")
    ...
    log_progress(f"Dimensiones del conjunto de entrenamiento (solo vivienda): {X_housing_train.shape}")
    log_progress(f"Dimensiones del conjunto de prueba (solo vivienda): {X_housing_test.shape}")
    log_progress(f"Dimensiones del conjunto de entrenamiento (con socioeconómicas): {X_all_train.shape}")
    log_progress(f"Dimensiones del conjunto de prueba (con socioeconómicas): {X_all_test.shape}")
    ...
    # ## 2. Construcción de modelos predictivos simplificados
    log_progress("\nConstruyendo modelos predictivos simplificados...")

    # Definir modelos a evaluar (versión simplificada)
    models = {
    ...
    def evaluate_model(model, X_train, X_test, y_train, y_test, model_name, dataset_name):
        ...
        log_progress(f"{model_name} en {dataset_name}:")
        log_progress(f"  RMSE: {rmse:.2f}")
        log_progress(f"  MAE: {mae:.2f}")
        log_progress(f"  R²: {r2:.4f}")
    ...
    for model_name, model in models.items():
        ...
        # Guardar el modelo
        best_models[f"{model_name}_housing"] = pipeline
    ...
    for model_name, model in models.items():
        ...
        # Guardar el modelo
        best_models[f"{model_name}_all"] = pipeline
    ...
    log_progress(f"Resultados guardados en {results_dir}/model_comparison_results.csv")
    ...
    log_progress(f"Gráfico RMSE guardado en {results_dir}/model_rmse_comparison.png")
    ...
    log_progress(f"Gráfico R² guardado en {results_dir}/model_r2_comparison.png")
    ...
    log_progress(f"Mejor modelo para análisis de importancia: {best_model_name} con {best_dataset}")
    ...
    if best_model_name == 'RandomForest':
        ...
        log_progress(f"Importancia de variables guardada en {results_dir}/feature_importance.csv")
        ...
        log_progress(f"Gráfico de importancia guardado en {results_dir}/feature_importance_top20.png")
        ...
        if best_dataset == "Con Socioeconómicas":
            ...
            log_progress(f"Gráfico de importancia por tipo guardado en {results_dir}/feature_importance_by_type.png")
            ...
            log_progress(f"Informe de importancia por tipo guardado en {results_dir}/importance_by_type.txt")
    else:
        log_progress(f"El modelo {best_model_name} no proporciona importancia de variables directamente.")
        ...
        if best_model_name == 'Ridge':
            ...
            log_progress(f"Importancia de coeficientes guardada en {results_dir}/coefficient_importance.csv")
            ...
            log_progress(f"Gráfico de coeficientes guardado en {results_dir}/coefficient_importance_top20.png")
    ...
    log_progress(f"Mejor modelo general: {best_overall_model_name} con {best_overall_dataset}")
    log_progress(f"R²: {results_df.loc[best_overall_idx, 'r2']:.4f}")
    log_progress(f"RMSE: {results_df.loc[best_overall_idx, 'rmse']:.2f}")
    log_progress(f"MAE: {results_df.loc[best_overall_idx, 'mae']:.2f}")
    ...
    log_progress(f"R²: {results_df.loc[best_housing_idx, 'r2']:.4f}")
    log_progress(f"RMSE: {results_df.loc[best_housing_idx, 'rmse']:.2f}")
    log_progress(f"MAE: {results_df.loc[best_housing_idx, 'mae']:.2f}")

    log_progress(f"\nMejor modelo con variables socioeconómicas: {best_all_model_name}")
    log_progress(f"R²: {results_df.loc[best_all_idx, 'r2']:.4f}")
    log_progress(f"RMSE: {results_df.loc[best_all_idx, 'rmse']:.2f}")
    log_progress(f"MAE: {results_df.loc[best_all_idx, 'mae']:.2f}")
    ...
    log_progress(f"R²: +{r2_improvement:.2f}%")
    log_progress(f"RMSE: -{rmse_improvement:.2f}%")
    ...
    log_progress(f"Gráfico de distribución de residuos guardado en {results_dir}/residuals_distribution.png")
    ...
    log_progress(f"Gráfico de residuos vs predichos guardado en {results_dir}/residuals_vs_predicted.png")
    ...
    log_progress(f"Gráfico de valores reales vs predichos guardado en {results_dir}/actual_vs_predicted.png")
        ...
        if 'importance_df' in locals():
            ...
            for _, row in importance_df.head(10).iterrows():
                if 'importance' in importance_df.columns:
                    ...
                    f.write(f"| {row['feature']} | {row['abs_coefficient']:.4f} |\n")
            
            if best_dataset == "Con Socioeconómicas" and 'housing_importance' in locals():
    ...
    log_progress("Análisis y modelado completados con éxito.")
    ...
    for model_name, model in best_models.items():
        with open(f"{results_dir}/{model_name}.pkl", 'wb') as f:
            pickle.dump(model, f)
    log_progress(f"Modelos guardados en el directorio {results_dir}")
    ...
    logging.error(f"Error en el script: {str(e)}")
